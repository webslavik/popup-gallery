...
(function() {
    ...
    function SkySlider(selector = null, options = null) {
        ...
        if (typeof selector != 'string') {
            console.error(`${selector} - is not correct selector.`);
            return;
        }
        ...
        if (options instanceof Object) {
            _.options = extendDefaults(_.options, options);
        }
    ...
    function close() {
        _.overlay.addEventListener('click', e => {
            ...
            if (
                !target.classList.contains('skySlider-overlay') &&
                !target.classList.contains('skySlider-slider-wrap')
            ) {
                return false;
            }
    ...
    function moveSlider() {
        ...
        _.sliderBelt.addEventListener('mouseup', e => {
            ...
            const DRAG__OFFSET =
                liElements[_.currentIndex].querySelector('img').offsetWidth *
                0.3;
    ...
    function changeImg(dir = null) {
        if (dir) {
            _.currentIndex = dir == 'left'
                ? Math.min(_.currentIndex + 1, _.elementsNumber - 1)
                : Math.max(_.currentIndex - 1, 0);
        }
    ...
    function moveThumbnailsBelt() {
        ...
        function onMouseMove(e) {
            ...
            if (
                totalDist <=
                -(
                    thumbnailWidth * _.elementsNumber -
                    thumbnailWidth * self.options.thumbnailsItemCount
                )
            ) {
                self.thumbnailsList.style.transform = `translate3d(-${thumbnailWidth *
                    _.elementsNumber -
                    thumbnailWidth * self.options.thumbnailsItemCount}px,0,0)`;
            } else if (totalDist >= 0) {
    ...
    function goToCurrentThumbnail() {
        ...
        let beltOffset =
            _.currentIndex - Math.floor(_.options.thumbnailsItemCount / 2);

        if (
            beltOffset >= 0 &&
            beltOffset <= _.elementsNumber - _.options.thumbnailsItemCount
        ) {
            _.thumbnailsList.style.transform = `translate3d(-${beltOffset *
                thumbnailWidth}px,0,0)`;
        }
    }
        ...
        for (let property in properties) {
            if (properties.hasOwnProperty(property)) {
                if (property === 'thumbnailsItemCount') {
                    if (properties[property] < 3)
                        source[property] = properties[property] = 3;
                    if (properties[property] > 8)
                        source[property] = properties[property] = 8;
                }
